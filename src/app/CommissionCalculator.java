...
public class CommissionCalculator implements iCommissionCalculator 
{
	...
	public double calculateCommission() 
	{
		...
		/**
		 * DZ final double commissionRatesForProbationaryEmployee[] = { 2, 3, 1, 3 };
		 * DZ Corrected incorrect commission rates for probationary employee
		 * DZ Constant were inputed in percentages 2, 3, 1, 3 
		 * DZ They were supposed to be in decimal representation of percentages 0.02, 0.03, 0.01, 0.03
		 */
		...
		for (SalesTransaction s : this.transactions) 
		{
			...
			{	
				commission += s.getTransactionAmount() * commissionTable[s.getTransactionType()];
			} 
			...
			else if ((netSales + s.getTransactionAmount()) >= minimumSalesForCommission) 
			{
				...
				// commission.
				double commissionableAmount = (netSales + s.getTransactionAmount()) - minimumSalesForCommission;
				netSales += s.getTransactionAmount();
				commission += commissionableAmount * commissionTable[s.getTransactionType()];
			} 
			
			else {netSales += s.getTransactionAmount();} // No commission. Simply go on.
		}
